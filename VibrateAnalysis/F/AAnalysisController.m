...
- (void) configureGraph {
    ...
    //Set graph title
    NSString *title = @"Magnitude and Phase Analysis";
    graph.title = title;
...
- (void) configurePlots {
    ...
    mpPlot = [CPTBarPlot tubularBarPlotWithColor:[CPTColor orangeColor] horizontalBars:NO];
    mpPlot.dataSource = self;
    mpPlot.identifier = @"M/P Analysis";
    mpPlot.barWidth = CPTDecimalFromDouble(0.1);
    
    // 3 - Set up plot space
    [plotSpace setYRange: [CPTPlotRange plotRangeWithLocation:CPTDecimalFromFloat( -0.5 ) length:CPTDecimalFromFloat( 3.0 )]];
    [plotSpace setXRange: [CPTPlotRange plotRangeWithLocation:CPTDecimalFromFloat( -5 ) length:CPTDecimalFromFloat( 10 )]];
